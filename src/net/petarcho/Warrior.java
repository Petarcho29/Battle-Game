...
public class Warrior extends Character{
    ...
    public Warrior() {
        super("net.petarcho.Warrior", 1200);
        gadget = false;
    }

    protected void basic(Character target) {
        System.out.println("net.petarcho.Warrior dealt 120 damage to " + target.type);
        target.health -= 120;
    }

    protected boolean ultimate(Character target) {
        if (this.energy >= 50) {
            System.out.println("net.petarcho.Warrior dealt 260 damage to " + target.type);
            target.health -= 260;
            this.energy -= 50;
            return false;
        }
